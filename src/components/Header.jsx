...
export default function Header({ planets, handleActivePlanet, menuState, setMenuState }) {
         ...
         <nav>
            ...
            <ul className="nav-list" ref={navlist}>
               {planets.map((planet, index) => {
                  return (
                     <li className={`
                     nav-link ${planet.name.toLowerCase()} ${planetIsActive === planet.name.toLowerCase() ? "active" : ""}`}
                     key={index}>
                        <a
                           href="#"
            ...
            <button className={`hamburger mobile ${menuState ? 'menu-close' : ''}`} onClick={() => setMenuState(!menuState)}>
               {/* <img src="images/icon-hamburger.svg" alt="mobile menu button"/> */}
               <svg xmlns="http://www.w3.org/2000/svg" width="24" height="17"><g ref={hamburgerIcon} fill="#FFF" fill-rule="evenodd"><path d="M0 0h24v3H0zM0 7h24v3H0zM0 14h24v3H0z"/></g></svg>
            </button>
         </nav>
      </header>
   );
